...
# ---------------- SIDEBAR INPUTS ----------------
st.sidebar.title("Aircraft Inputs")

aircraft_name = st.sidebar.text_input("Aircraft Name", "My Aircraft")
airfoil = st.sidebar.selectbox(
    "Airfoil",
    ["NACA0012", "NACA2412", "NACA4412", "NACA23012"]
)

mach = st.sidebar.number_input("Mach", value=0.3)
rho = st.sidebar.number_input("Density (kg/m³)", value=1.225)
span = st.sidebar.number_input("Wing Span", value=10.0)
area = st.sidebar.number_input("Wing Area", value=12.0)
alpha = st.sidebar.number_input("AoA (deg)", value=5.0)

# ---------------- CALCULATIONS ----------------
def compute(mach, rho, span, area, alpha):
    velocity = mach * 343
    AR = span**2 / area
...
AR, CL, CD, lift, drag, LD = compute(mach, rho, span, area, alpha)
...
def plot_airfoil(airfoil_name):
    if "2412" in airfoil_name:
        m, p, t = 0.02, 0.4, 0.12
    elif "0012" in airfoil_name:
        m, p, t = 0.0, 0.0, 0.12
    elif "4412" in airfoil_name:
        m, p, t = 0.04, 0.4, 0.12
    elif "23012" in airfoil_name:
        m, p, t = 0.02, 0.3, 0.12
    else:
        m, p, t = 0.0, 0.0, 0.12
